...
const handleRecived = async () => {
  console.log("FixedComponents:", FormData.FixedComponents); // Debugging output
  console.log("ReplacedComponents:", FormData.ReplacedComponents);

  const FixedComponents = Array.isArray(FormData.FixedComponents) ? FormData.FixedComponents.join(', ') : '';
    const ReplacedComponents = Array.isArray(FormData.ReplacedComponents) ? FormData.ReplacedComponents.join(', ') : '';
  try {
      // Prepare data to be sent to the API
      const issueData = {
          AssetID: searchParams.AssetID,
...
const [FormData, setFormData] = useState({
  ...
  RepairInvoiceNumber: '',
  RepairCost: '',
  FixedComponents: [], // Ensure this is initialized as an array
  ReplacedComponents: [], // Ensure this is initialized as an array
  RepairNote: '',
});
        ...
        <Dialog open={openReceived} onClose={handleClose} fullWidth maxWidth="sm">
          ...
          <DialogContent>
            ...
            <div className="divider-container">
              ...
              <span className="divider-text">ReplacedComponents and Fixed components</span>
              <hr className="section-divider" />
            </div>
              
            <Autocomplete
              multiple
              id="fixed-components-select"
              options={devicecomponents} // This needs to have all possible components
              getOptionLabel={(option) => option.component}
              onChange={(event, newValue) => {
                setFormData({...FormData, FixedComponents: newValue.map((item) => item.component)});
              }}
              renderInput={(params) => (
                <TextField {...params} label="Fixed Components" placeholder="Choose..." />
              )}
            />
            ...
            <Autocomplete
              ...
              style={{ width: '100%' }}
              renderInput={(params) => (
                <TextField {...params} label="ReplacedComponents" placeholder="Replaced Components" />
              )}
            />
